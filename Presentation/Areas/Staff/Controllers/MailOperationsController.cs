...
namespace Presentation.Areas.Staff.Controllers
{
    ...
    public class MailOperationsController : Controller
    {
        ...
        [HttpGet]
        public IActionResult SendForAdmins()
        {
            return View();
        }

        [HttpPost]
        public IActionResult SendForAdmins(SendForAdminsViewModel sendForAdminsViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    _sendForAdminsMail.SendMessage(admin.FullName, admin.Email, sendForAdminsViewModel.Subject, sendForAdminsViewModel.Content);
                }
                ...
                EmailsSent emailsSent = new EmailsSent
                {
                    SentFor = "Admins",
                    Date = DateTime.Now,
                    Content = sendForAdminsViewModel.Content
                };

                emailsSentManager.TInsert(emailsSent);

                TempData["SuccessMessage"] = "Mail Was Sent For Admins!";
                return RedirectToAction("MailOperations", "Staff");
            }

            else
            {
                return View(sendForAdminsViewModel);
            }
        }
